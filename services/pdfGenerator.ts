...
const prepareData = (data: UserData & InactionTemplateData, letterType: LetterType) => {
    ...
    if (isUntaetigkeitsklage) {
        ...
        processedData.BMV_BLOCK = data.alsoAppliedFiktion ? 'Ich weise darauf hin, что mir ohne einen Aufenthaltstitel oder eine Fiktionsbescheinigung Leistungen nach dem AsylbLG verweigert werden, was eine Notsituation für mich darstellt.' : '';
...
// Универсальный fetch: ищет и в /templates/, и в /assets/templates/
async function fetchTemplateFile(templateFile: string): Promise<Response> {
    let response = await fetch(`/templates/${templateFile}`);
    if (response.ok) return response;
    response = await fetch(`/assets/templates/${templateFile}`);
    if (response.ok) return response;
    throw new Error("Не удалось загрузить шаблон: File not found");
}

const generateDocxBlob = async (
    letterType: LetterType,
    templateFile: string,
    data: UserData & InactionTemplateData
): Promise<Blob> => {
    const response = await fetchTemplateFile(templateFile);
    if (!response.ok) {
